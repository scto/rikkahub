...
JNICALL Java_com_alibaba_mnnllm_android_llm_LlmSession_initNative(JNIEnv *env,
        jobject thiz,
        jstring modelDir,
...
JNICALL Java_com_alibaba_mnnllm_android_llm_LlmSession_submitNative(JNIEnv *env,
        jobject thiz,
        jlong llmPtr,
        jstring inputStr,
...
JNICALL Java_com_alibaba_mnnllm_android_llm_LlmSession_submitFullHistoryNative(
        JNIEnv *env,
        jobject thiz,
        jlong llmPtr,
...
Java_com_alibaba_mnnllm_android_llm_LlmSession_resetNative(JNIEnv
*env,
jobject thiz,
        jlong
object_ptr) {
...
JNIEnv *env;
if (jvm->GetEnv(reinterpret_cast
...
env->
SetFloatArrayRegion(audioDataArray,
0, size, data);
jboolean result = env->CallBooleanMethod(global_ref, onAudioDataMethod,
        audioDataArray, is_end);
env->
DeleteLocalRef(audioDataArray);
env->
DeleteLocalRef(listenerClass);
if (needDetach) {
jvm->
...
JNIEXPORT jstring
JNICALL
        Java_com_alibaba_mnnllm_android_llm_LlmSession_getDebugInfoNative(JNIEnv * env, jobject
thiz,
jlong objecPtr
) {
...
JNIEXPORT void JNICALL
Java_com_alibaba_mnnllm_android_llm_LlmSession_releaseNative(JNIEnv
*env,
jobject thiz,
        jlong
objecPtr) {
MNN_DEBUG("LIFECYCLE: About to DESTROY LlmSession at %p", reinterpret_cast
<void *>(objecPtr)
);
auto *llm = reinterpret_cast<mls::LlmSession *>(objecPtr);
delete
llm;
MNN_DEBUG("LIFECYCLE: LlmSessionInner DESTROYED at %p", reinterpret_cast
<void *>(objecPtr)
);
}
...
extern "C"
JNIEXPORT void JNICALL
Java_com_alibaba_mnnllm_android_llm_LlmSession_updateMaxNewTokensNative(JNIEnv
*env,
jobject thiz,
        jlong
llm_ptr,
...
extern "C"
JNIEXPORT void JNICALL
Java_com_alibaba_mnnllm_android_llm_LlmSession_updateSystemPromptNative(JNIEnv
*env,
jobject thiz,
        jlong
llm_ptr,
...
if (llm) {
llm->
setSystemPrompt(system_prompt_cstr);
}
env->
ReleaseStringUTFChars(system_promp_j, system_prompt_cstr
);
}

extern "C"
JNIEXPORT void JNICALL
Java_com_alibaba_mnnllm_android_llm_LlmSession_updateAssistantPromptNative(JNIEnv
*env,
jobject thiz,
        jlong
llm_ptr,
...
if (llm) {
llm->
SetAssistantPrompt(assistant_prompt_cstr);
}
env->
ReleaseStringUTFChars(assistant_prompt_j, assistant_prompt_cstr
);
}
}
extern "C"
JNIEXPORT void JNICALL
Java_com_alibaba_mnnllm_android_llm_LlmSession_updateEnableAudioOutputNative(JNIEnv
*env,
jobject thiz, jlong
llm_ptr,
jboolean enable
...
JNIEXPORT jstring
JNICALL
        Java_com_alibaba_mnnllm_android_llm_LlmSession_getSystemPromptNative(JNIEnv * env, jobject
thiz,
jlong llm_ptr
) {
auto *llm = reinterpret_cast<mls::LlmSession *>(llm_ptr);
...
extern "C"
JNIEXPORT void JNICALL
Java_com_alibaba_mnnllm_android_llm_LlmSession_clearHistoryNative(JNIEnv
*env,
jobject thiz,
        jlong
llm_ptr) {
...
return env->
NewObject(resultClass, resultCtor, testInstance, JNI_FALSE,
        env
->NewStringUTF("LLM session is not initialized"));
}
...
MNN_ERROR("JNI Exception: GetMethodID for getPrefillUs failed."); env->
...
MNN_ERROR("JNI Exception: GetMethodID for getDecodeUs failed."); env->
...
MNN_ERROR("JNI Exception: GetMethodID for getSamplesUs failed."); env->
...
MNN_ERROR("JNI Exception: FindClass for java/util/List failed."); env->
...
MNN_ERROR("JNI Exception: GetMethodID for List.add failed."); env->
...
MNN_ERROR("JNI Exception: CallObjectMethod for getPrefillUs failed."); env->
...
MNN_ERROR("JNI Exception: CallObjectMethod for getDecodeUs failed."); env->
...
MNN_ERROR("JNI Exception: CallObjectMethod for getSamplesUs failed."); env->
...
MNN_ERROR("JNI Exception: FindClass for java/lang/Long failed."); env->
...
MNN_ERROR("JNI Exception: GetMethodID for Long constructor failed."); env->
...
cppCallback.
onProgress = [env, globalCallback, onProgressMethod, progressClass, progressCtor](
    ...
    if (onProgressMethod && progressClass && progressCtor) {
        ...
        progressInfo.currentIteration,
                (jint)
        progressInfo.totalIterations,
                (jint)
        progressInfo.nPrompt,
                (jint)
        progressInfo.nGenerate,
...
cppCallback.
onError = [env, globalCallback, onErrorMethod](const std::string &error) {
    if (onErrorMethod) {
...
cppCallback.
onIterationComplete = [env, globalCallback, onProgressMethod, progressClass, progressCtor](
    ...
    if (onProgressMethod && progressClass && progressCtor) {
        ...
        0, (jint)
        0, (jint)
        0, // default values for structured fields
...
(int)result.prefill_times_us.

size(),

(int)result.decode_times_us.
...
MNN_ERROR("JNI Exception after NewObject for prefill."); break; }
env->
CallBooleanMethod(prefillUsList, addMethod, prefillLong
);
if (env->
...
MNN_ERROR("JNI Exception after add for prefill."); break; }
env->
DeleteLocalRef(prefillLong);
}

for (
long decode_time
: result.decode_times_us) {
...
MNN_ERROR("JNI Exception after NewObject for decode."); break; }
env->
CallBooleanMethod(decodeUsList, addMethod, decodeLong
);
if (env->
...
MNN_ERROR("JNI Exception after add for decode."); break; }
env->
DeleteLocalRef(decodeLong);
}

for (
long sample_time
: result.sample_times_us) {
...
MNN_ERROR("JNI Exception after NewObject for sample."); break; }
env->
CallBooleanMethod(samplesUsList, addMethod, sampleLong
);
if (env->
...
MNN_ERROR("JNI Exception after add for sample."); break; }
env->
DeleteLocalRef(sampleLong);
}
}
}
...
env->
DeleteGlobalRef(globalTestInstance);
...
return env->
NewObject(resultClass, resultCtor, testInstance, (jboolean)
