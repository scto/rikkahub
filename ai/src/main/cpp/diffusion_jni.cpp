...
Java_com_alibaba_mnnllm_android_llm_DiffusionSession_resetNative(JNIEnv
*env,
jobject thiz,
        jlong
instance_id) {
...
extern "C"
JNIEXPORT void JNICALL
Java_com_alibaba_mnnllm_android_llm_DiffusionSession_releaseNative(JNIEnv
*env,
jobject thiz,
        jlong
instance_id) {
...
env->
ReleaseStringUTFChars(extra_config_j, extra_json_config_cstr
);
env->
ReleaseStringUTFChars(config_path, config_path_cstr
);
...
Run(prompt,
        ...
        iter_num,
        random_seed,
[env, progress_listener, onProgressMethod](
int progress
) {
if (
progress_listener &&onProgressMethod
) {
jstring javaString = env->NewStringUTF(std::to_string(progress).c_str());
env->
CallBooleanMethod(progress_listener, onProgressMethod, javaString
);
env->
DeleteLocalRef(javaString);
}
});
...
env->
CallObjectMethod(hashMap, putMethod, env
->NewStringUTF("total_timeus"), env->
NewObject(env
->FindClass("java/lang/Long"), env->
GetMethodID(env
->FindClass("java/lang/Long"), "<init>", "(J)V"), duration));
return
hashMap;
}
